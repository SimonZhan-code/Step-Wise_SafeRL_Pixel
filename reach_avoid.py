...
from modules.models import Encoder, ObservationModel, RewardModel, TransitionModel, ValueModel_RA, bottle, CostModel, Controller_stoch_RA
from modules.planner import TargetSet_Func, AvoidSet_Func
from utils.utils import FreezeParameters, lambda_return, lineplot, write_video, imagine_ahead, l_loss, g_loss, reach_avoid_value_return
...
parser.add_argument('--cost_threshold', type=float, default=0.01, help='Threshold to delinate avoid set')
parser.add_argument('--reward_threshold', type=float, default=1.0, help='Threshold to delinate target set')
...
parser.add_argument('--ddqn_learning_rate', type=float, default=4e-4, metavar='α', help='Learning rate')
parser.add_argument('--l_learning_rate', type=float, default=8e-4, metavar='α', help='Learning rate')
parser.add_argument('--controller_learning_rate', type=float, default=1e-3, metavar='α', help='Learning rate')
parser.add_argument('--g_learning_rate', type=float, default=8e-4, metavar='α', help='Learning rate')
parser.add_argument(
...
l_func = TargetSet_Func(args.belief_size, args.state_size, args.hidden_size, args.dense_activation_function).to(
    device=args.device
)

g_func = AvoidSet_Func(args.belief_size, args.state_size, args.hidden_size, args.dense_activation_function).to(
    device=args.device
)

controller = Controller_stoch_RA(args.belief_size, args.state_size, args.hidden_size, env.action_size).to(
    device=args.device
)

value_model = ValueModel_RA(args.belief_size, args.state_size, args.hidden_size, args.dense_activation_function).to(
    device=args.device
)
...
print("transition, observation, reward, encoder, reach-avoid-controller, value models are ready")
...
value_optimizer = optim.Adam(
    ...
    lr=0 if args.learning_rate_schedule != 0 else args.ddqn_learning_rate,
    eps=args.adam_epsilon,
)
    ...
    for s in tqdm(range(args.collect_interval)):
        ...
        with FreezeParameters(model_modules):
            imagination_traj = imagine_ahead(
                ra_states, ra_beliefs, controller, transition_model, args.planning_horizon
            )
            
        imged_beliefs, imged_prior_states, imged_prior_means, imged_prior_std_devs = imagination_traj
        # print(imged_prior_states.size())
        
        # Calculate the RA set categorization loss and 
        # Retrieve imageined costs and reward to update the RA set categorization
        with FreezeParameters(model_modules):
        ...
        # Reach-Avoid implementation: value loss calculation and optimization
