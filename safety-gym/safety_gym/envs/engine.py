...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    def build_goal_position(self):
        ...
        goal_body_id = self.model.body_name2id('goal')
        self.model.body_pos[goal_body_id][:2] = self.layout['goal']
        self.sim.forward()
    ...
    def obs_lidar_pseudo(self, positions):
        ...
        for pos in positions:
            ...
            temp = self.ego_xy(pos)
            # print(temp)
            z = np.complex128(temp[0]+temp[1]*1j)  # X, Y as real, imaginary components
            dist = np.abs(z)
